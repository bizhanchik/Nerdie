          ...
          messages: [
            {
              role: "system",
              content:
                "You are a helpful assistant that generates concise lecture titles. Output only the title, nothing else.",
            },
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a helpful study assistant. You output only valid JSON.",
            },
            { role: "user", content: prompt },
          ],
  ...
  async generatePersonalizedLesson(
    ...
    const personalizationContext =
      userProfile && userProfile.interests.length > 0
        ? `
The student is ${
            userProfile.age ? `${userProfile.age} years old and` : ""
          } interested in: ${userProfile.interests.join(", ")}.
...
Create a comprehensive, engaging lesson with the following structure in JSON format:
{
  ...
  "content": "Detailed lesson content in markdown format. Include:
    ...
    - Examples and analogies ${
      personalizationContext ? "related to the student's interests" : ""
    }
...
${
  personalizationContext
    ? "8. Use examples and analogies that relate to the student's interests whenever natural"
    : ""
}`;
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert educational content creator who creates engaging, personalized lessons. Output only valid JSON.",
            },
            { role: "user", content: prompt },
          ],
          response_format: { type: "json_object" },
  ...
  async extractTextFromPhotos(photoUris: string[]): Promise<string> {
    if (!API_KEY) {
      throw createAIError(new Error("API Key not set"), "transcription");
    }
    ...
    try {
      // Process all photos and extract text
      const imagePromises = photoUris.map(async (uri) => {
        // Create File instance and read as base64
        const file = new File(uri);
        const base64 = await file.base64();

        // Determine image type
        const imageType = uri.endsWith(".png") ? "image/png" : "image/jpeg";
          ...
          messages: [
            {
              ...
              content: [
                {
                  type: "text",
                  text: "Extract all text from these images. Include all handwritten notes, diagrams, equations, and any visible text. Format the output as a continuous text document that preserves the structure and content. If there are multiple pages, separate them clearly.",
                },
                ...imageContents,
              ],
            },
      ...
      console.error("Photo text extraction error:", error);
      throw createAIError(error, "transcription");
    }
  },

  async generateStudyMaterialsWithPhotos(
    transcription: string,
    photoUris?: string[]
  ): Promise<ProcessingResponse> {
    if (!API_KEY) {
    ...
    if (photoUris && photoUris.length > 0) {
      try {
        // Extract text from photos for context
        photoContext = await this.extractTextFromPhotos(photoUris);
      } catch (error) {
        console.warn(
          "Failed to extract text from photos, continuing without photo context",
          error
        );
      }
    }

    const fullContext = photoContext
      ? `${transcription}\n\nADDITIONAL CONTEXT FROM NOTES:\n${photoContext}`
      : transcription;

    return this.generateStudyMaterials(fullContext);
  },
};
